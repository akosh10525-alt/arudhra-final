...
pages = convert_from_path(r"arudhra project\Radar precie.pdf", 300)
...
llm = Ollama(model="mistral")        # ~3B params, very lightweight  
# or
llm = Ollama(model="llama2:3b")      # smaller version of Llama2
# or
llm = Ollama(model="gemma:2b")       # Google’s tiny model

# Load Spacy modelc
nlp = spacy.load("en_core_web_sm")
llm = Ollama(model="mistral", options={"num_ctx": 1024})
...
POPPLER_PATH = r"D:\poppler-25.07.0\Library\bin"  # Update this path to your Poppler bin directory
...
# Path to poppler and tesseract
POPPLER_PATH = r"D:\poppler-25.07.0\Library\bin"
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"

def extract_text_from_chart(img_path):
    """Extract axis labels and numbers from chart image"""
...
def extract_charts_from_pdf(pdf_path):
    ...
    for i, page in enumerate(pages):
        ...
        chart_text = extract_text_from_chart(img_path)
        if chart_text.strip():
            chart_texts.append({
                "page": i+1,
                "text": chart_text
...
def generate_questions_from_chart(chart_text, llm):
    prompt = f"""
    The following chart data was extracted from a PDF:

    {chart_text}

    Based on this chart, generate:
    - 2 Multiple Choice Questions
    - 1 Fill in the Blank
    - 1 True/False
    - 1 Long Answer

    Provide both questions and answers.
    """
...
def process_pdf(file, start_page, end_page, question_counts, question_type):
    ...
    try:
        ...
        # Step 3: Generate text-based questions
        generated_questions = generate_questions(extracted_text, question_counts, question_type)
        output += "📝 Text-based Questions:\n" + generated_questions + "\n"

        # Step 4: Extract and process charts
        charts = extract_charts_from_pdf(file_path)
        if charts:
            output += "\n\n📊 Chart-based Questions:\n"
            for chart in charts:
                output += f"\n--- Page {chart['page']} ---\n"
                output += generate_questions_from_chart(chart["text"], llm) + "\n"

        if not output.strip():
            output = "⚠️ No valid text or charts found in PDF."
...
def generate_from_ui(pdf_file, q_type, q_count):
    return process_pdf(pdf_file.name, q_type, q_count)
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Dropdown(["MCQ", "Fill in the Blanks", "True/False", "Long Answer"], label="Question Type"),
        gr.Slider(1, 10, step=1, value=5, label="Number of Questions")
    ],
    outputs="text",
    title="PDF → Exam Question Generator",
    description="Uploads a PDF and generates exam-style questions from both text and charts."
)
